...
export default class GameAPI{
    ...
    register(params){
        ...
        new Promise((resolve, reject) => {
            t.servicePost(route, pars, authToken, (err, result) => {
    ...
    servicePost( route:string, params, accessToken, callback)  {
        var url = this.serviceURL + route;
        let finalParams = JSON.stringify(params)

        const promise = this.postHttp( url, finalParams, 'application/json', accessToken);
    ...
    postHttp(url, pars, contentType = 'application/json', accessToken = null): Promise<any> {
